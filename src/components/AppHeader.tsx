            ...
            {navItems.map((item) => {
              ...
              return (
                <Link
                  key={item.to}
                  to={item.to}
                  className={`relative flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium transition-colors ${isActive ? "text-primary-foreground" : "text-muted-foreground hover:text-foreground"
                    }`}
                >
                  {isActive && (
                    <motion.div
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                key={item.to}
                to={item.to}
                className={`relative flex flex-col items-center justify-center min-w-[64px] h-12 transition-colors ${isActive ? "text-primary" : "text-muted-foreground"
                  }`}
              >
                {isActive && (
                  <motion.div
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link
              key={item.to}
              to={item.to}
              className={`relative flex flex-col items-center justify-center min-w-[64px] h-12 transition-colors ${isActive ? "text-primary-foreground" : "text-muted-foreground"
                }`}
            >
